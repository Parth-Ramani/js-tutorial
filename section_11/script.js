...
movements.forEach((movement) => (movement += movement));
console.log(movements);
...
const displayMovement = function (currmov) {
  containerMovements.innerHTML = "";
  console.log(containerMovements.innerHTML);
console.log(`display: ${currmov}`)
  currmov.forEach((movement, i) => {
...
const calcPrintBalance = function (currAcc) {
  currAcc.balance = currAcc.movements.reduce((acc, mov) => {
    return acc + mov;
  });
...
const calcDisplaySummary = function (currAcc) {
  ...
  const outcome = currAcc.movements
    .filter((mov) => {
      return mov < 0;
    })
    .reduce((acc, mov) => acc + mov, 0);
